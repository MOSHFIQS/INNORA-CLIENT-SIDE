...
import { useGetBannerByIdQuery, useUpdateBannerMutation } from '@/redux/api/settingApi';
...
export default function EditBannerPage() {
     ...
     const bannerId = typeof params?.id === 'string' ? params.id : Array.isArray(params?.id) ? params.id[0] : '';
     const router = useRouter();

     const { data: banner, isLoading: isFetching } = useGetBannerByIdQuery(bannerId, { skip: !bannerId });
     ...
     if (!banner) {
          return (
               <div className="p-12 text-center bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-gray-800">
                    <p className="text-xs text-gray-500 font-semibold">Hero banner slide record not found.</p>
                    <Link href="/dashboard/banners" className="text-xs text-[#b99d75] font-bold underline mt-2 block">
                         Return to Carousel Management
                    </Link>
               </div>
          );
     }
     ...
     const handleSubmit = async (e) => {
          ...
          try {
               ...
               toast.error(err?.data?.message || 'Failed to update banner slide');
          }
     };
               ...
               <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-gray-800 p-6 shadow-xs space-y-4">
                         <MediaUploader
                              ...
                              hint="High resolution panoramic image (1920x1080 recommended)"
                         />
                    </div>
